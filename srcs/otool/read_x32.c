...
/*   Updated: 2017/03/03 17:01:40 by frmarinh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_nm_otool.h"

void	text_section_x32(struct segment_command *segment, \
	t_file *file, struct section *section)
{
	void	*ptr;
	int		i;
	int		count;
	...
	while (i < header->ncmds)
	{
		...
		if (cmd->cmd == LC_SEGMENT)
		{
			...
			if (ft_strcmp(section->segname, "__TEXT") == 0)
				text_section_x32(segment, file, section);
		}
		ptr += cmd->cmdsize;
		i++;
	}
